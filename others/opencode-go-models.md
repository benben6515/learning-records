...
qwen3.6-plus, deepseek-v4-pro, deepseek-v4-flash, **hy3** …）做 1:1 join。同時取每個 model 的
...
- 資料抓取日：**2026-07-27**（OpenRouter snapshot；上次 2026-07-17）
  - 本輪變動：➕ **Hy3**（Tencent）新上架；**MiMo-V2.5-Pro / DeepSeek V4 Pro** Go 單價從 1.74/3.48 降到 0.435/0.87（與 OR 同步）；**Grok 4.5** cacheR 0.50→0.30；**GLM-5.2 / Kimi K2.7 / K2.6** OR 價下陞；**DeepSeek V4 Flash** OR 漲到與 Go 同價；**MiniMax M2.5** 已從推薦清單移除（仍計價、仍可用）。

---

## 🗺 目前可用模型 Overview（速查）

> 共 **17 個模型**（🆕 = 本輪新增）。按 **Go 計量表 output 單價（$/1M）** 由便宜到貴排列；月額度只有 $15 的 4 個以 ⚠️ 標出（重度使用會很快撞牆）。
>
> 計價單位：per 1M tokens（$）。`In/Out` = Go 計量表 input / output 單價。
...
| **MiMo-V2.5-Pro**     | `mimo-v2.5-pro`     | ⚠️ **$15** | 1,048,576 | 0.435 / 0.87     |    ❌     | 單價降 4×（原 1.74/3.48）；但 $15 額度不變 |
| **DeepSeek V4 Pro**   | `deepseek-v4-pro`   | ⚠️ **$15** | 1,048,576 | 0.435 / 0.87     |    ❌     | 單價降 4×；但 $15 額度不變             |
...
| **Qwen3.7 Max**       | `qwen3.7-max`       |    $60     | 1,000,000 | 2.50 / 7.50      |    ❌     | Qwen 旗艦；**無 vision**               |
...
- **⚠️ 月額度只有 $15**（會撞牆）→ Grok 4.5 / Kimi K3 / MiMo-V2.5-Pro / DeepSeek V4 Pro
- **最貴 → 最便宜**：Kimi K3（$3/$15）> Grok 4.5（$2/$6）> Qwen3.7 Max > GLM-5.x > Qwen3.6 Plus > … > Hy3（$0.14/$0.58）> MiMo-V2.5 / DeepSeek V4 Flash（$0.14/$0.28）
...
- **⚠️ 每個模型的「月使用價值」不一定是 $60**：官方定價表有 `Usage` 欄，多數模型 $60，但有 4 個只有 **$15/月**（無折扣 / 已是破盤價，見下表與下方「為何額度較低」）。其中 Grok 4.5（600/mo）與 Kimi K3（490/mo）因 $15 + 高單價 request 極少；MiMo-V2.5-Pro / DeepSeek V4 Pro 雖單價已降到 $0.435/$0.87，但 $15 額度仍把它們上限壓在 ~16–17K。
...
| --------------------- | --------: | ----------------------- | ---------------------------- | :----: | ---------------------- |
| **GLM-5.2** `glm-5.2` | 1,048,576 | 1.40 / 4.40 / 0.26 / –  | 0.7308 / 2.2968 / 0.1357 / – |  $60   | Go 比 OR 貴 ~1.92×     |
...
| **Kimi K2.6** `kimi-k2.6`           |   262,144 | 0.95 / 4.00 / 0.16  | 0.6460 / 2.7200 / 0.1088 |   $60   | Go 比 OR 貴 ~1.47×（OR 大降）   |
...
| **MiMo-V2.5-Pro** `mimo-v2.5-pro` | 1,048,576 | 0.435 / 0.87 / 0.003625 | 0.4350 / 0.8700 / 0.0036 | **$15** | **Go = OR 完全一致**；與 DeepSeek V4 Pro 同價（單價降 4×） |
...
| Model                                     |   Context | Go In/Out/CacheR     | OR In/Out/CacheR         | 月額度  | 備註                                    |
| ----------------------------------------- | --------: | -------------------- | ------------------------ | :-----: | --------------------------------------- |
| **DeepSeek V4 Pro** `deepseek-v4-pro`     | 1,048,576 | 0.435 / 0.87 / 0.003625 | 0.4350 / 0.8700 / 0.0036 | **$15** | **Go = OR 完全一致**；與 MiMo-V2.5-Pro 同價同 ctx（單價降 4×） |
| **DeepSeek V4 Flash** `deepseek-v4-flash` | 1,048,576 | 0.14 / 0.28 / 0.0028 | 0.1400 / 0.2800 / 0.0028 |   $60   | **Go = OR 完全一致**；最便宜，request 數最多                  |
...
只有 **Qwen3.7 Plus** 與 **Qwen3.6 Plus** 有分段計價；其餘模型（含 1M ctx 的 GLM-5.2、Grok 4.5、MiniMax M3、DeepSeek V4、MiMo、Kimi K3、Hy3）不論 context 多長都是單一費率。
...
- **8 / 17 支援 vision**：Grok 4.5、Kimi K3 / K2.7 Code / K2.6、MiMo-V2.5、MiniMax M3、Qwen3.7 / 3.6 Plus。
...
- 全部 17 個的 `output_modalities` 都只有 `text`——沒有任何一個能直接生圖/生影片。
...
> ⚠️ 線性刻度下，便宜的 Flash / MiMo-V2.5 是其他模型的 **~30–400×**，幾乎吃滿整條；最貴的 **Grok 4.5（600/mo）與 Kimi K3（490/mo）因 $15 額度 + 高單價，bar 短到只剩 1 格**。
...
3. **Go metering 與 OpenRouter 零售價的關係**（本輪後幾乎全部收斂到 Go = OR）：
   - **完全一致（Go = OR，等於直付 provider）**：Grok 4.5、Kimi K3、MiniMax M3、MiMo-V2.5、**MiMo-V2.5-Pro**、**DeepSeek V4 Pro/Flash**（後三者本輪降價後同步）。
   - **小幅溢價**：GLM-5.2 / 5.1（~1.9×）、Kimi K2.7 Code（~1.3×）、**Kimi K2.6**（~1.47×，本輪 OR 大降後 Go 反而變貴）、Qwen 系列、**Hy3**（~1.06×）。
   - **「4× 溢價」類別已消失**：上一版 MiMo-V2.5-Pro / DeepSeek V4 Pro 的 Go 單價是 4× OR（1.74/3.48 vs 0.435/0.87），本輪 Go 降到與 OR 同步。
   - 重度用戶若超額改扣 Zen 餘額，現在 Go 與 OR 幾乎同價，差別只剩訂閱槓桿；唯獨 **GLM-5.x / Kimi K2.6** 走 OR 直付會比 Go 省。
...
5. **Context 上限一覽**：1M 級 = GLM-5.2、Kimi K3、MiniMax M3、MiMo V2.5/Pro、DeepSeek V4 Pro/Flash、Qwen3.7 Max/Plus、Qwen3.6 Plus；500K = Grok 4.5；262K = Kimi K2.6/K2.7 Code、Hy3；~200K = GLM-5.1、MiniMax M2.7/M2.5。
6. **最貴 → 最便宜**：Kimi K3（$3/$15）> Grok 4.5（$2/$6）> Qwen3.7 Max > Qwen3.6 Plus > … > Hy3（$0.14/$0.58）> DeepSeek V4 Flash / MiMo-V2.5（$0.14/$0.28，最便宜且 request 數最多）。
