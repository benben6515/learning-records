...
```
Token 檔：~/.local/share/slk/tokens/T026DBXKU2H.json   (0600)
...
### Permalink 反解（重要技巧）
`https://moldintelcoltd.slack.com/archives/D0A6RF44V98/p1784705289473959`
→ channel = `D0A6RF44V98`，ts = `1784705289.473959`（p 後每 6 位補一個點）
→ 精準抓單則：
...
1. **上班**：捷徑選單（閃電）→ modal 表單（工作事項+地點）→ 送出
   - app DM（`D0B4UJJKJKF`）收到「打卡完成」+ **下班按鈕**
     （attachments→blocks→actions→button，`action_id` 是 UUID）
   - 同步公告到 `#punch-channel`（`C0291GHQK1D`）
2. **下班**：點 DM 裡那顆按鈕 → 完成訊息 + "clicked *下班卡*"
...
herdr pane send-text wA:pK "test-benben" # 打頻道名
...
- **punch-remind loop**：`~/ai/loop/punch-remind.sh` + launchd
  `com.benben.punch-remind`（週一至五 18:35 觸發、隨機 18:40–18:59 發
  「記得打卡唷 (◑‿◐)」到 `C0AAEEDPG4F`）。token 就讀上面的 token 檔，
  curl 直呼 REST（腳本內小 payload 不經 rtk 沒問題）
